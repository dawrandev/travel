@extends('layouts.main')

@section('content')
<div class="section-header">
    <h1>Вопросы</h1>
    <div class="section-header-breadcrumb">
        <div class="breadcrumb-item"><a href="{{ route('dashboard') }}"><i class="fas fa-home"></i> Панель управления</a></div>
        <div class="breadcrumb-item active">Вопросы</div>
    </div>
</div>

<div class="row">
    <div class="col-12">
        <div class="card">
            <div class="card-header">
                <h4>Список вопросов</h4>
            </div>
            <div class="card-body">
                <!-- Search and Filter -->
                <div class="row mb-3">
                    <div class="col-md-5">
                        <div class="input-group">
                            <input type="text" id="searchInput" class="form-control" placeholder="Поиск по имени, email, телефону..." value="{{ request('search') }}">
                            <div class="input-group-append">
                                <button class="btn btn-primary" type="button" id="searchBtn">
                                    <i class="fas fa-search"></i>
                                </button>
                            </div>
                        </div>
                    </div>
                    <div class="col-md-3">
                        <select id="statusFilter" class="form-control">
                            <option value="">Все статусы</option>
                            <option value="pending" {{ request('status') == 'pending' ? 'selected' : '' }}>В ожидании</option>
                            <option value="answered" {{ request('status') == 'answered' ? 'selected' : '' }}>Отвечено</option>
                        </select>
                    </div>
                    <div class="col-md-2">
                        <button type="button" id="resetBtn" class="btn btn-secondary btn-block">
                            <i class="fas fa-redo"></i> Сбросить
                        </button>
                    </div>
                </div>

                <div id="questionsTableContainer">
                    @include('pages.questions.table')
                </div>
            </div>
        </div>
    </div>
</div>
@endsection

@push('modals')
<!-- View Modal -->
<div class="modal fade" id="viewModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Детали вопроса</h5>
                <button type="button" class="close" data-dismiss="modal">
                    <span>&times;</span>
                </button>
            </div>
            <div class="modal-body" id="questionDetails">
                <!-- Details will be loaded here -->
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-dismiss="modal">Закрыть</button>
            </div>
        </div>
    </div>
</div>

<!-- Edit Status Modal -->
<div class="modal fade" id="editStatusModal" tabindex="-1" role="dialog">
    <div class="modal-dialog modal-sm" role="document">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Изменить статус</h5>
                <button type="button" class="close" data-dismiss="modal">
                    <span>&times;</span>
                </button>
            </div>
            <form id="statusForm" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <div class="form-group">
                        <label>Статус</label>
                        <select name="status" id="statusSelect" class="form-control" required>
                            <option value="pending">В ожидании</option>
                            <option value="answered">Отвечено</option>
                        </select>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Отмена</button>
                    <button type="submit" class="btn btn-primary">Сохранить</button>
                </div>
            </form>
        </div>
    </div>
</div>
@endpush

@push('scripts')
<script>
    $(document).ready(function() {
        // Search function
        function loadQuestions() {
            const search = $('#searchInput').val();
            const status = $('#statusFilter').val();

            $.ajax({
                url: '{{ route("questions.index") }}',
                type: 'GET',
                data: {
                    search: search,
                    status: status
                },
                success: function(response) {
                    $('#questionsTableContainer').html(response);
                },
                error: function(xhr) {
                    console.error('Error loading questions:', xhr);
                }
            });
        }

        // Search button
        $('#searchBtn').click(function() {
            loadQuestions();
        });

        // Enter key on search input
        $('#searchInput').keypress(function(e) {
            if (e.which == 13) {
                loadQuestions();
            }
        });

        // Status filter change
        $('#statusFilter').change(function() {
            loadQuestions();
        });

        // Reset button
        $('#resetBtn').click(function() {
            $('#searchInput').val('');
            $('#statusFilter').val('');
            loadQuestions();
        });

        // View question (event delegation)
        $(document).on('click', '.view-question', function() {
            const questionId = $(this).data('id');

            $.ajax({
                url: '/questions/' + questionId + '/show',
                type: 'GET',
                success: function(question) {
                    const tourTitle = question.tour && question.tour.translations.length > 0 ?
                        (question.tour.translations.find(t => t.lang_code === 'ru') || question.tour.translations[0]).title :
                        'Тур удален';

                    const statusLabels = {
                        'pending': 'В ожидании',
                        'answered': 'Отвечено'
                    };

                    let html = `
                        <table class="table table-bordered">
                            <tr>
                                <th style="width: 40%;">ID</th>
                                <td>${question.id}</td>
                            </tr>
                            <tr>
                                <th>Тур</th>
                                <td>${tourTitle}</td>
                            </tr>
                            <tr>
                                <th>Имя</th>
                                <td>${question.user_name}</td>
                            </tr>
                            <tr>
                                <th>Email</th>
                                <td>${question.email}</td>
                            </tr>
                            <tr>
                                <th>Телефон 1</th>
                                <td>${question.phone_primary}</td>
                            </tr>
                            <tr>
                                <th>Телефон 2</th>
                                <td>${question.phone_secondary || '-'}</td>
                            </tr>
                            <tr>
                                <th>Вопрос</th>
                                <td>${question.comment || '<span class="text-muted">Нет вопроса</span>'}</td>
                            </tr>
                            <tr>
                                <th>Статус</th>
                                <td>${statusLabels[question.status] || question.status}</td>
                            </tr>
                            <tr>
                                <th>Создано</th>
                                <td>${new Date(question.created_at).toLocaleString('ru-RU')}</td>
                            </tr>
                            ${question.updated_at !== question.created_at ? `
                            <tr>
                                <th>Обновлено</th>
                                <td>${new Date(question.updated_at).toLocaleString('ru-RU')}</td>
                            </tr>
                            ` : ''}
                        </table>
                    `;

                    $('#questionDetails').html(html);
                    $('#viewModal').modal('show');
                },
                error: function(xhr) {
                    console.error('Error loading question:', xhr);
                }
            });
        });

        // Edit status (event delegation)
        $(document).on('click', '.edit-status', function() {
            const questionId = $(this).data('id');
            const currentStatus = $(this).data('status');

            $('#statusForm').attr('action', '/questions/' + questionId + '/status');
            $('#statusSelect').val(currentStatus);
            $('#editStatusModal').modal('show');
        });

        // Pagination links (event delegation)
        $(document).on('click', '.pagination a', function(e) {
            e.preventDefault();
            const url = $(this).attr('href');
            const search = $('#searchInput').val();
            const status = $('#statusFilter').val();

            $.ajax({
                url: url,
                type: 'GET',
                data: {
                    search: search,
                    status: status
                },
                success: function(response) {
                    $('#questionsTableContainer').html(response);
                },
                error: function(xhr) {
                    console.error('Error loading page:', xhr);
                }
            });
        });
    });
</script>
@endpush

