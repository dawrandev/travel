...
class ContactBannerService
{
    ...
    public function create(array $data)
    {
        ...
        $banner = $this->contactBannerRepository->create(['is_active' => $data['is_active']]);

        // Handle 3 images
        if (isset($data['images']) && is_array($data['images'])) {
            foreach ($data['images'] as $index => $image) {
                $imagePath = $image->store('uploads', 'public');
                $this->contactBannerRepository->createImage($banner->id, [
    ...
    public function update(int $id, array $data)
    {
        ...
        $this->contactBannerRepository->update($banner, ['is_active' => $data['is_active']]);
